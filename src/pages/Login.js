...
const Login = () => {
  ...
  const handleSendOtp = () => {
    // Here, you can send the OTP to the provided mobile number using Firebase
    const auth = firebase.auth();
    auth.signInWithPhoneNumber(mobileNumber).then(() => {
      setOtpSent(true);
    });
  };

  const handleVerifyOtp = () => {
    // Here, you can verify the OTP using Firebase
    const auth = firebase.auth();
    auth
      .signInWithCredential(new firebase.auth.PhoneAuthCredential(otpSent, otp))
      .then(() => {
        nav("/basic-details");
        setTimeout(() => {
          setOtpSent(false);
