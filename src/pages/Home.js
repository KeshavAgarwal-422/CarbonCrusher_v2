...
const Home = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-start w-full h-full">
      <h2 className="w-full text-[6vw] pl-[15vw] py-[1vh] text-left">
        Hi, Keshav
      </h2>
      <DateSelector />
      ...
      <div className="w-[90%] flex overflow-x-scroll">
        {transportModeArray.map((item, index) => (
          <div
            key={index}
            className="bg-[#272727] flex flex-col justify-between h-[45vw] min-w-[30vw] p-[4vw] mx-[2vw] my-[2vw] rounded-xl shadow-md "
          >
            <div className="text-[5vw] flex justify-between ">
              <span>{item.category}</span>
              {item.icon}
            </div>
            <div className="flex items-center justify-between gap-[5vw] text-yellow-400">
              {" "}
              <div className="text-[7vw]">
                {" "}
                <IoTimeSharp></IoTimeSharp>
              </div>
              <span className="text-[4vw]">{item.duration} hrs</span>
            </div>
            <div className="flex items-center justify-between gap-[5vw] text-blue-600">
              <div className="text-[7vw]">
                <GiPathDistance></GiPathDistance>
              </div>
              <span className="text-[4vw]"> {item.distance} km</span>
            </div>
            <div className="flex items-center justify-between gap-[5vw] text-red-500">
              <div className="text-[10vw] flex items-center">
                <MdCo2></MdCo2>
              </div>
              <span className="text-[4vw]"> {item.carbonConsumption} kg</span>
            </div>
          </div>
        ))}
      </div>
