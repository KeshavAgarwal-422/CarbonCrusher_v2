...
function App() {
  ...
  const tabs = [
    ...
    {
      name: "notifications",
      icon: <IoNotificationsSharp></IoNotificationsSharp>,
    },
  ...
  return (
    <div className="h-screen w-screen bg-[#141414] text-[#E0FF63] font-outfit relative overflow-y-auto">
      ...
      {isLoggedIn ? (
        <>
          {" "}
          <div className="fixed bottom-0 left-0 w-full h-[8vh] bg-[#141414] flex justify-around items-center rounded-s-xl">
            {tabs.map((tab) => (
              <div
                key={tab.name}
                className={`text-[3vh] ${
                  activeTab === tab.name ? "text-[#E0FF63]" : "text-white"
                } bg-transparent`}
                onClick={() => {
